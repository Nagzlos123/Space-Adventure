...
public class KosmosData : MonoBehaviour
{
    ...
    void Start()
    {
        if (storeData.artefact1PanelActive)
        {
            artefact1Panel.SetActive(true);
            LoadGameState();
        }

        if (storeData.artefact2PanelActive)
        {
            
            artefact2Panel.SetActive(true);
            LoadGameState();
        }
       

        if (storeData.artefact3PanelActive)
        {
            //storeData.playButtonOn = false;
            artefact3Panel.SetActive(true);
            LoadGameState();
        }

        if (storeData.artefact4PanelActive)
        {
            //storeData.playButtonOn = false;
            artefact4Panel.SetActive(true);
            LoadGameState();
        }
